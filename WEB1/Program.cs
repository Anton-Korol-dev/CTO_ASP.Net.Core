...
app.MapGet("/", (ApplicationContext db) => db.Workers.ToList());
...
app.MapGet("/workersbyphone/{phonenumber}", async (string workerphonenumber, ApplicationContext db) =>
{
    Worker? workers = await db.Workers.FirstOrDefaultAsync(u => u.Worker_Phone_Number == workerphonenumber);
    if (workers == null)
    {
        return Results.NotFound(new { message = "Робітника по номеру телефона не знайдено" });
    }
    return Results.Json(workers);
});

app.MapPost("/addworkers", (string firstname, string middlename, string lastname, string workerphonenumber, ApplicationContext db) =>
{
    Worker workers = new Worker();
    workers.ID = Guid.NewGuid().ToString();
    workers.FirstName = firstname;
    workers.MiddleName = middlename;
    workers.LastName = lastname;
    workers.Worker_Phone_Number = workerphonenumber;
